...
void set_params_fprop(Flash_fwd_params &params,
                      // sizes
                      const size_t b, const size_t b_k, const size_t seqlen_q,
    ...
    // Convert p from float to int so we don't have to convert the random uint
...
#ifdef FLASHATTENTION_ENABLE_2D
    window_size_left = std::min(int(width), window_size_left);
    window_size_right = std::min(int(width), window_size_right);
    window_size_top = std::min(int(height), window_size_top);
    window_size_bottom = std::min(int(height), window_size_bottom);

    if (window_size_left < 0)
        window_size_left = width;
    if (window_size_right < 0)
        window_size_right = width;
    if (window_size_top < 0)
        window_size_top = height;
    if (window_size_bottom < 0)
        window_size_bottom = height;
    params.window_size_left = window_size_left;
    params.window_size_right = window_size_right;
    params.window_size_top = window_size_top;
    params.window_size_bottom = window_size_bottom;
    params.height = height;
    params.width = width;
    params.is_causal = false;
    params.is_local =
        (window_size_left < int(width) || window_size_right < int(width) ||
         window_size_top < int(height) || window_size_bottom < int(height));
#else
    ...
    if ((window_size_left < int(seqlen_k) ||
         window_size_right < int(seqlen_k)) &&
        !params.is_causal) {
...
void set_params_dgrad(
    ...
    at::Tensor dv, void *cu_seqlens_q_d, void *cu_seqlens_k_d, void *seqused_q,
...
#ifdef FLASHATTENTION_ENABLE_2D
    int window_size_top, int window_size_bottom, const size_t height,
    const size_t width,
#endif
    bool deterministic) {

    set_params_fprop(params, b, b, seqlen_q, seqlen_k, seqlen_q_rounded,
                     seqlen_k_rounded, h, h_k, d, d_rounded, q, k, v, out,
                     cu_seqlens_q_d, cu_seqlens_k_d, seqused_q, seqused_k,
                     nullptr, softmax_lse_d, p_dropout, softmax_scale,
                     window_size_left, window_size_right
#ifdef FLASHATTENTION_ENABLE_2D
                     ,
                     window_size_top, window_size_bottom, height, width
#endif
    );
...
// 80% of the best efficiency.
inline int num_splits_heuristic(int batch_nheads_mblocks, int batch_nheads,
                                int num_SMs, int num_n_blocks, int max_splits,
                                int head_size, bool use_one_mma_wg) {
    // Goal of the starting threshold is to determine whether to split or not.
    // Empirically, the efficiency threshold can be much lower than 80%
    // depending on num_n_blocks.
    ...
    if (head_size == 128) {
        ...
        if (num_n_blocks <= 40) {
            ...
            start_threshold =
                .33f +
                std::max(0.f, (std::log2f(num_n_blocksf) - std::log2f(50)) *
                                  0.02971f);
        } else {
            // Just split freely
            start_threshold = .8f;
    ...
    // first_wave); Only use start_threshold if initial work doesn't exceed one
    // wave
    if ((first_wave / ceil(first_wave) > start_threshold &&
         first_wave <= 1.f) ||
        (first_wave / ceil(first_wave) > .8f)) {
        return 1;
    }
    // if (first_wave_batch_nheads > start_threshold) { return 1; }
    // if (first_wave_batch_nheads > start_threshold || first_wave > .8f) {
    // return 1; } if (float(batch_nheads)/num_SMs > start_threshold) { return
    // 1; }
    ...
    if (num_n_blocks < 8 || (use_one_mma_wg && num_n_blocks < 10)) {
        return 1;
    }
    ...
    // NOTE: disable split eligibility check for FA3 since we have dynamic tile
    // scheduler for exiting splits with no work early, and check leads to
    // efficiency quantization issues. Comment from FA2: Some splits are not
    // eligible. For example, if we have 64 blocks and choose 11 splits, we'll
    // have 6 * 10 + 4 blocks. If we choose 12 splits, we'll have 6 * 11 + (-2)
    // blocks (i.e. it's 11 splits anyway). So we check if the number of blocks
    // per split is the same as the previous num_splits. auto ceildiv = [](int
    // a, int b) { return (a + b - 1) / b; }; auto is_split_eligible =
    // [&ceildiv, &num_n_blocks](int num_splits) {
    ...
    for (int num_splits = 1; num_splits <= max_splits; num_splits++) {
        ...
        // printf("num_splits = %d, n_waves = %f, ceil(n_waves) = %f,  eff =
    ...
    // Empirically, efficiency threshold in these cases is about 40% for 64K
    // seqlen_k
    float threshold =
        num_m_blocks == 1 ? std::min(0.3f + batch_nheads * 0.1f, 0.8f) : 0.8f;
    ...
    for (int num_splits = 1; num_splits <= max_splits; num_splits++) {
        ...
        if (efficiency[num_splits - 1] > threshold) {
            // printf("num_splits chosen = %d, threshold = %f, efficiency =
...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(
    ...
    const int num_heads_k, const int head_size, const int max_seqlen_k,
    const int max_seqlen_q, const int head_size_rounded, const float p_dropout,
    const int num_splits, cudaDeviceProp *dprops, bool use_gqa_packing,
    bool is_causal, struct c10::TensorOptions opts) {
    ...
    if (p_dropout == 0.0f) { // SplitKV is not implemented for dropout
        if (num_splits < 1) {
            ...
            const int block_h = 1 << static_cast<int>(std::ceil(
                                    std::log2(std::clamp(gqa_ratio, 1, 32))));
            ...
            const int batch_nheads = use_gqa_packing ? batch_size * num_heads_k
            ...
            params.num_splits = num_splits_heuristic(
                batch_nheads_mblocks, batch_nheads, dprops->multiProcessorCount,
                num_n_blocks, 128, head_size, use_one_mma_wg);
...
mha_fwd(at::Tensor &q,       // batch_size x seqlen_q x num_heads x head_size
    ...
    TORCH_CHECK(is_sm90,
                "FlashAttention-3 only supports Hopper GPUs or newer.");
    ...
    TORCH_CHECK(head_size_og == 64 || head_size_og == 128 ||
                    head_size_og == 256,
    ...
    if (out_.has_value()) {
        out = out_.value();
        // TORCH_CHECK(out.dtype() == q_dtype, "Output must have the same dtype
        // as inputs");
        TORCH_CHECK(
            q_dtype == at::ScalarType::Float8_e4m3fn
    ...
    set_params_fprop(params, batch_size, batch_size, seqlen_q, seqlen_k,
                     seqlen_q_rounded, seqlen_k_rounded, num_heads, num_heads_k,
                     head_size, head_size_rounded, q_padded, k_padded, v_padded,
                     out,
...
#ifdef FLASHATTENTION_ENABLE_2D
                     ,
                     /*window_size_top=*/window_size_top,
                     /*window_size_bottom=*/window_size_bottom,
                     /*height=*/height,
                     /*width=*/width
#endif
    );
...
std::vector<at::Tensor> mha_varlen_fwd(
    ...
    c10::optional<at::Tensor>
        &seqused_q, // b. If given, only this many elements of each batch
    ...
    if (seqused_q.has_value()) {
        ...
        TORCH_CHECK(seqused_q_.dtype() == torch::kInt32,
    ...
    auto opts = q.options();
    auto softmax_lse =
        torch::empty({num_heads, total_q}, opts.dtype(at::kFloat));

    Flash_fwd_params params;
    set_params_fprop(
        params, batch_size, batch_size, max_seqlen_q, max_seqlen_k,
        seqlen_q_rounded, seqlen_k_rounded, num_heads, num_heads_k, head_size,
        head_size_rounded, q_padded, k_padded, v_padded, out, cu_seqlens_q_d,
    ...
    // printf("mha_varlen_fwd: params.seqlen_k=%d, max_seqlen_k=%d,
    // params.page_num_blocks=%d\n", (int)params.seqlen_k, (int)max_seqlen_k,
    // (int)params.page_num_blocks);
    if (max_seqlen_k > 0) {
        auto stream = at::cuda::getCurrentCUDAStream().stream();
        run_mha_fwd(params, stream);
    } else {
...
std::vector<at::Tensor> mha_bwd(
    ...
    set_params_dgrad(
        ...
        /*cu_seqlens_k_d=*/nullptr,
        /*seqused_q=*/nullptr,
        /*seqused_k=*/nullptr, dq_accum.data_ptr(),
    ...
    if (head_size_og % 8 != 0) {
        dq = dq.index({"...", torch::indexing::Slice(torch::indexing::None,
                                                     head_size_og)});
        dk = dk.index({"...", torch::indexing::Slice(torch::indexing::None,
                                                     head_size_og)});
        dv = dv.index({"...", torch::indexing::Slice(torch::indexing::None,
                                                     head_size_og)});
    }
...
std::vector<at::Tensor> mha_varlen_bwd(
    ...
    auto softmax_d = torch::empty({num_heads, total_q_padded_rounded},
                                  opts.dtype(at::kFloat));
    auto softmax_lse_log2 = torch::empty({num_heads, total_q_padded_rounded},
                                         opts.dtype(at::kFloat));
    at::Tensor dq_accum;
    at::Tensor dk_accum, dv_accum;
    dq_accum =
    ...
    if (max_seqlen_q > 0) {
        ...
        // If max_seqlen_q == 0, then we have an empty tensor. We need to set
        // the output to 0.
        dk_expanded.zero_();
        dv_expanded.zero_();
        softmax_d.zero_();
...
std::vector<at::Tensor> mha_fwd_kvcache(
    ...
    int num_splits, int max_seqlen_k_hint, bool use_gqa_packing) {
    ...
    if (!paged_KV) {
        ...
        CHECK_SHAPE(kcache, num_blocks, page_block_size, num_heads_k,
                    head_size_og);
        CHECK_SHAPE(vcache, num_blocks, page_block_size, num_heads_k,
                    head_size_og);
        CHECK_SHAPE(block_table, batch_size, max_num_blocks_per_seq);
    }
    ...
    Flash_fwd_params params;
    set_params_fprop(
        params, batch_size, batch_size_c, seqlen_q, seqlen_k, seqlen_q_rounded,
        seqlen_k_rounded, num_heads, num_heads_k, head_size, head_size_rounded,
    ...
    if (k_.has_value()) {
        ...
        if (head_size_og % 8 != 0) {
            k_padded = torch::nn::functional::pad(
                k, torch::nn::functional::PadFuncOptions(
                       {0, 8 - head_size_og % 8}));
            v_padded = torch::nn::functional::pad(
                v, torch::nn::functional::PadFuncOptions(
                       {0, 8 - head_size_og % 8}));
        } else {
            k_padded = k;
            v_padded = v;
    ...
    if (leftpad_k_.has_value()) {
        TORCH_CHECK(!paged_KV, "We don't support Paged KV and leftpad_k "
                               "running at the same time yet");
    ...
    std::tie(softmax_lse_accum, out_accum) = set_params_splitkv(
        params, batch_size, num_heads, num_heads_k, head_size,
        max_seqlen_k_hint, seqlen_q, head_size_rounded, /*dropout*/ 0.f,
        num_splits, dprops, use_gqa_packing, is_causal, opts);

    auto tile_count_semaphore =
        is_causal || params.is_local || params.num_splits != 1
            ? torch::zeros({1}, opts.dtype(torch::kInt32))
            : torch::empty({1}, opts.dtype(torch::kInt32));
    params.tile_count_semaphore = tile_count_semaphore.data_ptr<int>();
    ...
    TORCH_CHECK(!alibi_slopes_.has_value(),
                "Alibi Slopes are not supported yet");
    // set_params_alibi(params, alibi_slopes_, batch_size, num_heads);

    auto stream = at::cuda::getCurrentCUDAStream().stream();
    // Only split kernel supports appending to KV cache, or indexing to the
    // cache with cache_batch_idx, or paged KV cache run_mha_fwd(params, stream,
    ...
    if (head_size_og % 8 != 0) {
        ...
        if (k_.has_value()) {
            ...
            // size not divisible by 8, but we don't expect to get this case in
            // practice. This is just so that the code works for that case.
