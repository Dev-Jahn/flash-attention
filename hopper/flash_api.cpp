                                ...
                                int max_splits, int head_size, bool use_one_mma_wg)
{
    ...
    if (head_size == 128)
    {
        if (std::log2f(num_n_blocksf) <= 4)
        { // 2048 -- .25
            start_threshold = .20f + (std::log2f(num_n_blocksf) - 3) * .05f;
        }
        else if (std::log2f(num_n_blocksf) <= 5)
        { // 4096 -- .25
            start_threshold = .25f;
        }
        else if (std::log2f(num_n_blocksf) <= 6)
        { // 8192 -- .36
            start_threshold = .28f + (std::log2f(num_n_blocksf) - 5) * .08f;
        }
        else if (std::log2f(num_n_blocksf) <= 7)
        { // 16K -- .42
