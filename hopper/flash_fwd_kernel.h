...
template <typename Ktraits, bool Is_causal, bool Is_local,
          typename TileScheduler, typename Seqlen_traits,
          typename Seqlen_traits_Q = Seqlen_traits>
__global__ void __launch_bounds__(Ktraits::kNWarps *cutlass::NumThreadsPerWarp,
                                  1)
    compute_attn_ws(CUTE_GRID_CONSTANT typename CollectiveMainloopFwd<
                        Ktraits, Is_causal, Is_local, Seqlen_traits,
                        Seqlen_traits_Q>::Params const mainloop_params,
                    CUTE_GRID_CONSTANT typename CollectiveEpilogueFwd<
                        Ktraits, Seqlen_traits_Q>::Params const epilogue_params,
                    CUTE_GRID_CONSTANT
    ...
    using CollectiveMainloop =
        CollectiveMainloopFwd<Ktraits, Is_causal, Is_local, Seqlen_traits,
                              Seqlen_traits_Q>;
    ...
    static_assert(Ktraits::kNWarps == 8 || Ktraits::kNWarps == 12 ||
                  Ktraits::kNWarps == 16);
    if (warp_group_idx == 0) { // Producer
        cutlass::arch::warpgroup_reg_dealloc<Ktraits::kNWarps == 12 ? 24
                                                                    : 32>();
        ...
        if (warp_idx_in_warpgroup == 0) { // Load Q, K, V
            ...
            for (auto work_tile_info = scheduler.get_initial_work();
                ...
                if constexpr (seqlen_traits_q.UseVarSeqLen) {
                    // NOTE: to support in future with gqa packed layouts,
                    // changed kBlockM to kBlockM/kBlockH
                    if (m_block * (kBlockM / kBlockH) >=
                ...
                collective_mainloop.get_n_block_min_max(
                    mainloop_params, m_block, n_split_idx, seqlen_traits_q,
                    seqlen_traits_k, n_block_min, n_block_max);
                if constexpr (Is_causal || Is_local ||
                              seqlen_traits_k.UseVarSeqLen ||
                              Ktraits::Is_split) {
                    if (n_block_max <= n_block_min) {
                        scheduler.prefetch_next_work(scheduler_params,
                                                     work_tile_info);
                ...
                collective_mainloop.load(
                    mainloop_params, pipeline_k, pipeline_v, smem_pipe_write_k,
                    smem_pipe_write_v, shared_storage, scheduler,
                    scheduler_params, work_tile_info, block_coord, work_idx,
                    seqlen_traits_q, seqlen_traits_k, n_block_min, n_block_max);
                ++work_idx;
            }
            collective_mainloop.load_tail(pipeline_k, pipeline_v,
                                          smem_pipe_write_k, smem_pipe_write_v);
        }
    } else { // Consumer
        cutlass::arch::warpgroup_reg_alloc<Ktraits::kNWarps == 16   ? 160
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            seqlen_traits_k.init(bidb);
            if constexpr (seqlen_traits_q.UseVarSeqLen) {
                // NOTE: to support in future with gqa packed layouts, changed
                // kBlockM to kBlockM/kBlockH
                if (m_block * (kBlockM / kBlockH) >=
                    seqlen_traits_q.actual_seq_len) {
                    continue;
                }
            ...
            collective_mainloop.get_n_block_min_max(
                ...
                seqlen_traits_k, n_block_min, n_block_max);
            if constexpr (Is_causal || Is_local ||
                          seqlen_traits_k.UseVarSeqLen || Ktraits::Is_split) {
                if (n_block_max <= n_block_min) { // We exit early and write 0
                                                  // to gO and -inf to gLSE.
                    if constexpr (!Seqlen_traits_Q::UseGQAPacking) {
                        collective_epilogue.store_zero(
                            epilogue_params, shared_storage,
            ...
            collective_mainloop.mma(
                mainloop_params, pipeline_k, pipeline_v, smem_pipe_read_k,
                smem_pipe_read_v, tOrO, softmax, n_block_min, n_block_max,
                threadIdx.x - NumCopyThreads, work_idx, m_block, shared_storage,
                seqlen_traits_q, seqlen_traits_k);
            // tOrO, softmax, n_block_max, threadIdx.x - NumCopyThreads +
            // (work_idx >> 30), work_idx, shared_storage);
            collective_epilogue.store(
                epilogue_params, tOrO, softmax.row_sum, shared_storage,
                tiled_mma1, threadIdx.x - NumCopyThreads, block_coord,
...
__global__ void __launch_bounds__(Ktraits::kNWarps *cutlass::NumThreadsPerWarp,
    ...
    compute_attn_ws_fp8(
        CUTE_GRID_CONSTANT typename CollectiveMainloopFwd<
            ...
            Seqlen_traits_Q>::Params const mainloop_params,
        CUTE_GRID_CONSTANT
        typename CollectiveEpilogueFwd<Ktraits, Seqlen_traits_Q>::Params const
            epilogue_params,
        CUTE_GRID_CONSTANT
        typename TileScheduler::Params const scheduler_params,
    ...
    // static constexpr int kBlockN = Ktraits::kBlockN;
    // static constexpr int kHeadDim = Ktraits::kHeadDim;
    static constexpr bool Delay_V_release =
        Is_causal && Ktraits::kHeadDim == 128 && Ktraits::kNWarps != 8;
    ...
    shared_storage.seqlen_init_k =
        seqlen_traits_k.UseVarSeqLen || bool(seqlen_traits_k.seq_used);
    ...
    static_assert(Ktraits::kNWarps == 8 || Ktraits::kNWarps == 12 ||
                  Ktraits::kNWarps == 16);
    if (warp_group_idx == 0) { // Producer
        cutlass::arch::warpgroup_reg_dealloc<Ktraits::kNWarps == 16   ? 32
                                             : Ktraits::kNWarps == 12 ? 40
                                                                      : 56>();
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            if (shared_storage.seqlen_init_k) {
                seqlen_traits_k.init_no_guard(bidb);
            }
            if constexpr (seqlen_traits_q.UseVarSeqLen) {
                // NOTE: to support in future with gqa packed layout, changed
                // kBlockM to kBlockM/kBlockH
                if (m_block * (kBlockM / kBlockH) >=
                    seqlen_traits_q.actual_seq_len) {
                    continue;
            ...
            collective_mainloop.load_fp8(
                ...
                smem_pipe_write, smem_pipe_read, shared_storage, scheduler,
                scheduler_params, work_tile_info, block_coord, work_idx,
                seqlen_traits_q, seqlen_traits_k, n_block_min, n_block_max);
            ++work_idx;
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            Tensor tOrO =
                partition_fragment_C(tiled_mma1, select<0, 2>(TileShape_MNK{}));
            flash::Softmax<2 * (2 * kBlockM / NumMmaThreads), Use_max_offset>
                softmax(shared_storage.softmax_scale_qk_log2);
            ...
            collective_mainloop.mma_fp8<Delay_V_release>(
                ...
                smem_pipe_release, tOrO, softmax, n_block_min, n_block_max,
                threadIdx.x - NumCopyThreads, work_idx, m_block, shared_storage,
                seqlen_traits_q, seqlen_traits_k);
